...
var callGetDownloadStatus = rpcDeclare('get_download_status', ['project','filename','upx']);
...
return view.extend({
    ...
    var mirror = (els.mirror || {}).value || self._s.mirror || 'github';
    var upx    = project !== 'luci-app-vnt2' && !!((document.getElementById('s-upx') || {}).checked);
    ...
    callDoUpdate(project, fname, upx, mirror).then(function(r) {
        if (!r || r.result !== 'ok') {
            ...
            if (els.log_content)
                els.log_content.textContent += '✗ ' + (r ? r.msg : '启动失败') + '\n';
            showMirrorRow(els.mirror_row, true);
            return;
        }
        var dots  = 0;
        var timer = setInterval(function() {
            dots++;
            if (els.log_content)
                els.log_content.textContent = '正在下载 ' + fname + ' ' + '.'.repeat(dots % 10 + 1) + '\n';
            callGetDownloadStatus(project, fname, upx).then(function(s) {
                if (!s || s.status === 'downloading'
                       || s.status === 'processing'
                       || s.status === 'pending') return;
                clearInterval(timer);
                if (els.btn) els.btn.disabled = false;
                if (s.status === 'done') {
                    if (els.log_content)
                        els.log_content.textContent = (s.msg || '') + '✓ 更新成功，已安装：' + (s.installed || '') + '\n';
                    showMirrorRow(els.mirror_row, false);
                } else {
                    if (els.log_content)
                        els.log_content.textContent = '✗ ' + (s.msg || '更新失败') + '，请切换镜像源重试\n';
                    showMirrorRow(els.mirror_row, true);
                }
            });
        }, 2000);
        ...
        if (els.btn) els.btn.disabled = false;
        if (els.log_content)
            els.log_content.textContent += '✗ 错误：' + String(err) + '\n';
        showMirrorRow(els.mirror_row, true);
    });
},
});